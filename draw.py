...
def draw(args):
    ...
    run_name = f'draw-{env_name}-{run_date}'
    ...
    for episode in tqdm(range(0, config.model.train_episodes)):
        ...
        if (episode + 1) % config.model.eval_interval == 0:
            ...
            print("Episode %d, Average steps %.2f, Average Speed %.2f, Crash rate %.2f"
                  % (episode + 1, avg_step, avg_speed_mean, crash_rate))
