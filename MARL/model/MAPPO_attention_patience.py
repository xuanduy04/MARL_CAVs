...
class MAPPO_attention_patience(BaseModel):
    def __init__(self, config: Config):
        super(MAPPO_attention_patience, self).__init__(config)
    ...
    def train(self, env: AbstractEnv, curriculum_training: bool, writer: SummaryWriter, global_episode: int):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                if self.global_step % self.patience == 0 or self.global_step == self.num_training_steps:
                    if self.curr_loss < self.best_state['loss']:
